...
public class CodesCommand extends AbstractCommand {
    ...
    public Mono<Void> handle(ChatInputInteractionEvent event) {
        ...
        if (runFor != null) {
            if (invoker.get().getRoleIds().stream().map(Snowflake::asLong).noneMatch(id -> config.getAllowCodesOtherRanks().contains(id))) return event.reply().withEphemeral(true).withEmbeds(Util.errorEmbed("You do not have permission to run this command for other users."));
