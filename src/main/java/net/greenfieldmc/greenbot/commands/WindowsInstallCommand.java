...
public class WindowsInstallCommand extends AbstractCommand {

    private final Plugin plugin;
    private final Config config;

    public WindowsInstallCommand(Plugin plugin, Config config) {
        super("How to install Greenfield on Windows", "windowsinstall");
        this.plugin = plugin;
        this.config = config;
    }
    ...
    public Mono<Void> handle(ChatInputInteractionEvent event) {
        ...
        if (runFor != null) {
            ...
            channel.createMessage(user.getMention() + " " + config.getWindowsInstallLink()).block();
            return event.reply().withEphemeral(true).withEmbeds(Util.goodEmbed("Sent Windows Install Video", "Sent Windows install video successfully."));
        }

        return event.reply(invoker.get().getMention() + " " + config.getWindowsInstallLink()).withEphemeral(true);
    }
    ...
    public ApplicationCommandRequest getCommandRequest() {
        return ApplicationCommandRequest.builder()
                ...
                .addOption(ApplicationCommandOptionData.builder()
                        .name("run_for")
                        .description("The discord user who needs to see the Windows install tutorial.")
